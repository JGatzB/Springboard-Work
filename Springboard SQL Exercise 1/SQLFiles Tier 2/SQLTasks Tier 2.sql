...
SELECT DISTINCT CONCAT(firstname, ' ', surname) AS member_name, Facilities.name AS court_name
FROM Members
...
def select_all_tasks(conn):
    ...
    CONCAT(m2.surname, ' ', m2.firstname) AS recommended_by_name
FROM
    Members m1
LEFT JOIN
...
def select_all_tasks(conn):
    ...
    f.name AS facility_name,
    COUNT(b.bookid) AS member_usage,
    EXTRACT(MONTH FROM b.starttime) AS booking_month
FROM
    Facilities f
JOIN
    Bookings b ON f.facid = b.facid
...
WHERE
    b.memid != 0  
GROUP BY
    facility_name, booking_month
ORDER BY
    booking_month, member_usage DESC;
        """
    cur.execute(query1)
